...
from src.node_editor.collector import NODES, get_class_from_opcode, LISTBOX_MIMETYPE 
...
class SubWindow(NodeEditorWidget):
    ...
    def onDrop(self, event):
        if event.mimeData().hasFormat(LISTBOX_MIMETYPE):
            ...
            try:
                node = get_class_from_opcode(op_code)(self.scene,parent = self.parent)
    ...
    def handleNewNodeContextMenu(self, event):
        ...
        if action is not None:
            new_calc_node = get_class_from_opcode(action.data())(self.scene,parent=self.parent)
