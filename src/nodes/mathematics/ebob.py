...
from src.node_editor.constants import NODE_MATHEMATICS_EBOB 
...
@register_node(NODE_MATHEMATICS_EBOB) 
class InputINTNode(Node): 
    width = 400
    height = 80 
    op_code = NODE_MATHEMATICS_EBOB
    op_title = "EBOB"
    ...
    def evalImplementation(self, name=None, data=None):
        ...
        if isinstance(a, (int, float)) and isinstance(b, (int, float)):
            ebob = self.gcd(a,b)
            self.sendData("Sonuç", ebob)
            self.content.edit.setText(str( ebob))
