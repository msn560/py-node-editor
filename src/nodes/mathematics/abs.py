...
from src.node_editor.constants import NODE_MATHEMATICS_ABS
from  src.node_editor.numeric_text_line import NumericLineEdit

class lineIntContent(Content):
    def initUI(self):
        super().initUI()   
        self.edit = NumericLineEdit(parent=self ,allow_float=True, allow_negative=True)
...
@register_node(NODE_MATHEMATICS_ABS) 
class InputINTNode(Node): 
    width = 400
    height = 80 
    op_code = NODE_MATHEMATICS_ABS
    op_title = "Mutlak Değer"
    content_label_objname = "line_int_subtrac"
    ...
    def evalImplementation(self, name=None, data=None):
        ...
        if isinstance(a, (int, float)) :
            self.sendData("Sonuç",abs(a))
            self.content.edit.setText( str(abs(a)))
        else:
